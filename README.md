...
| --- | --- | --- |
| [3DSForge](https://github.com/MaximEremenko/3DSForge) | Forward calculator: computes the 3-D diffuse intensity I(*hkl*) in reciprocal space directly from atomistic configurations (RMCProfile `.rmc6f`, LAMMPS data files, unified structure HDF5) and visualizes it as interactive 3-D isosurfaces, slice planes, and 2-D maps | Type-3 NUFFT on a user-defined *hkl* grid with three engines — WebGPU, CPU FFT, and direct CPU summation for verification; neutron, X-ray, and electron scattering tables; average-lattice subtraction and 3-D smoothing; exports to `.dat`, unified HDF5, ParaView `.vtk`, `.cube`, CSV/SVG, and standalone Plotly HTML |
